...
const STEP_SEQUENCE = [1, 2, 3, 4, 5, 6, 7, 8, 85, 9];
...
function getStepLabel(step) {
  ...
  return step === 85 ? `步骤 8.5：${title}` : `步骤 ${step}：${title}`;
}
...
const handlers = {
  ...
  async EXECUTE_SIGNUP_STEP(payload) {
    ...
    if (step === 85) {
      return runManagedStep(85, async () => {
        const result = await sendToActiveAuthTab({ type: 'STEP85_PHONE_VERIFICATION', payload: {} });
        if (result?.error) {
          throw new Error(result.error);
        }
        return result;
      }, {
        startMessage: '步骤 8.5：正在执行短信验证...',
        successMessage: '步骤 8.5：短信验证已完成',
      });
    }
      ...
      return runManagedStep(8, () => new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          const clickResult = await chrome.tabs.sendMessage(authTab.id, {
            ...
            payload: {
              enableSmsVerification: Boolean(state.enableSmsVerification),
            },
          });
          ...
          if (clickResult?.needsPhoneVerification) {
            resolve({ needsPhoneVerification: true });
            return;
          }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === 'STEP_COMPLETE') {
    Promise.all([
      ...
      addLog(`页面内步骤 ${message.step === 85 ? '8.5' : message.step} 已完成`, 'ok'),
    ])
      .then(() => {
  ...
  if (message?.type === 'STEP_ERROR') {
    ...
    Promise.all([
      ...
      (message.step === 2 || message.step === 3) && _sender?.tab?.id
        ? clearPendingSignupStep(_sender.tab.id)
        : Promise.resolve(),
      addLog(`页面内步骤 ${message.step === 85 ? '8.5' : message.step} 失败：${message.error || '未知错误'}`, 'error'),
    ])
      .then(() => {
  ...
  if (message?.type === 'GET_PENDING_SIGNUP_STEP') {
    ...
    readPendingSignupStep(tabId)
      .then((storedPayload) => sendResponse({ ok: true, data: storedPayload }))
      .catch((error) => sendResponse({ ok: false, error: error.message || String(error) }));
    return true;
  }
