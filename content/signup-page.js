...
const heroSms = globalThis.HotmailRegisterHeroSms;
...
function getPhoneCodeInput() {
  const candidates = document.querySelectorAll(
    'input[autocomplete="one-time-code"], input[name*="code" i], input[id*="code" i], input[inputmode="numeric"], input[maxlength="6"]'
  );

  return Array.from(candidates).find((element) => isVisibleElement(element)) || null;
}

function getPhoneSubmitButton() {
  const candidates = document.querySelectorAll('button, [role="button"], input[type="submit"]');
  return Array.from(candidates).find((element) => {
    if (!isVisibleElement(element)) return false;
    if (!isActionEnabled(element)) return false;
    return /发送|send|继续|continue|下一步|next|verify|验证|submit/i.test(getActionText(element));
  }) || null;
}
...
async function step85PhoneVerification() {
  if (!heroSms?.requestHeroPhoneNumber || !heroSms?.pollHeroSmsCode) {
    throw new Error('短信验证模块未就绪，请检查 content/get-code.js 是否已加载。');
  }
  ...
  if (!submitButton) {
    throw new Error(`步骤 8.5：未找到发送验证码按钮。URL: ${location.href}`);
  }
  ...
  if (verifyButton && isActionEnabled(verifyButton)) {
    await pauseForInteraction('beforePrimaryClick');
    utils.clickElement(verifyButton);
  }
  ...
  throw new Error('步骤 8.5：短信验证后未进入授权页，请检查页面状态。');
}

async function step8FindAndClick(payload = {}) {
  const enableSmsVerification = Boolean(payload.enableSmsVerification);
  const startedAt = Date.now();
  while (Date.now() - startedAt < 10000) {
    if (isAddPhonePageReady()) {
      if (enableSmsVerification) {
        return { ok: true, needsPhoneVerification: true };
      }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  const tasks = {
    EXECUTE_STEP: async () => {
      ...
      if (message.step === 8) return step8FindAndClick(message.payload || {});
      if (message.step === 85) return step85PhoneVerification();
    ...
    STEP8_FIND_AND_CLICK: async () => step8FindAndClick(message.payload || {}),
    STEP85_PHONE_VERIFICATION: async () => step85PhoneVerification(),
  };
