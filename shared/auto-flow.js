...
async function runStep85IfNeeded(executeSignupStep, step8Result) {
...
export async function runSingleAutoFlow({ actions = {} } = {}) {
  ...
  await checkAutoControl();
  await runStep85IfNeeded(executeSignupStep, step8Result);
  await checkAutoControl();
...
function getFirstIncompleteStep(stepStatuses = {}) {
  const orderedSteps = [1, 2, 3, 4, 5, 6, 7, 8, 85, 9];
...
export async function continueSingleAutoFlow({ state = {}, actions = {} } = {}) {
  ...
  await checkAutoControl();
  await addLog(`继续自动流程：从步骤 ${startStep === 85 ? '8.5' : startStep} 开始`);
