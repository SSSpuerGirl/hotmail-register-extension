...
const STEP_ORDER = [1, 2, 3, 4, 5, 6, 7, 8, 85, 9];
...
function hydrateForm(state) {
  ...
  $('enable-sms-verification').checked = Boolean(state.enableSmsVerification);
...
function renderSteps(state) {
  ...
  elements.stepsProgress.textContent = `${getCompletedCount(state)} / ${STEP_ORDER.length}`;
}

function extractStepTag(message) {
  const match = String(message || '').match(/(?:步骤|Step)\s*(\d+(?:\.5)?)/i);
  return match?.[1] || '';
}
...
function collectSettings() {
  return {
    ...
    enableSmsVerification: $('enable-sms-verification').checked,
...
bindAction('step-85', 'EXECUTE_SIGNUP_STEP', {
  payload: { step: 85 },
  saveFirst: true,
  successMessage: '步骤 8.5 已完成',
  loadingText: '执行中...',
});
